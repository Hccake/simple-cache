...
import com.hccake.simpleredis.operation.CacheDelOps;
import com.hccake.simpleredis.operation.CachePutOps;
import com.hccake.simpleredis.operation.CachedOps;
import com.hccake.simpleredis.operation.function.VoidMethod;
...
public class CacheStringAspect {
    ...
    public Object around(ProceedingJoinPoint point) throws Throwable {
        ...
        if(cachedAnnotation != null){
            ...
            //redis 分布式锁的 key
            String lockKey = key + GlobalCacheConfig.lockKeySuffix();
    ...
    private Consumer<Object> prodCachePutFunction(ValueOperations<String, String> valueOperations, String key, long ttl) {
        ...
        if (ttl < 0) {
            ...
            cachePut = value -> valueOperations.set(key, (String) value, GlobalCacheConfig.expireTime(), TimeUnit.SECONDS);
        } else {
    ...
    public Object cached(CachedOps ops) throws Throwable {
        ...
        if (CacheLock.lock(ops.lockKey(), reqId)) {
            ...
            if (cacheData == null) {
                ...
                cacheData = dbData == null ? GlobalCacheConfig.nullValue() : cacheSerializer.serialize(dbData);
                //设置缓存
    ...
    public Object cachePut(CachePutOps ops) throws Throwable {
        ...
        //将返回值放置入缓存中
        String cacheData = data == null ? GlobalCacheConfig.nullValue() : cacheSerializer.serialize(data);
        ops.cachePut().accept(cacheData);
